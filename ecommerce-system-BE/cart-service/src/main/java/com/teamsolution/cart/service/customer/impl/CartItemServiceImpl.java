...
public class CartItemServiceImpl implements CartItemService {
    ...
    // Customer
    @Override
    public CartItemResponse addCartItem(UUID customerId, CreateCartItemRequest request) {
    ...
    public void deleteCartItem(UUID customerId, UUID cartItemId) {
        ...
        cartItem.setStatusChangeReason(StatusChangeReason.MANUAL);
    ...
    public CartItemResponse updateCartItemQuantity(
        ...
        if (request.quantity() == 0) {
            ...
            item.setStatusChangeReason(StatusChangeReason.MANUAL);
    ...
    public CartItemResponse updateCartItemVariant(
        ...
        if (!oldItem.getProductId().equals(UUID.fromString(newVariant.getProductId()))) {
            throw new AppException(ErrorCode.INVALID_PRODUCT_VARIANT_CHANGE);
        }
    ...
    private CartItemResponse createNewCartItem(Cart cart, CreateCartItemRequest request, Variant variant) {
        validateStock(request.quantity(), variant.getStock());
    ...
    private void validateCartItemBelongsToCart(List<CartItem> list, CartItem cartItem) {
        if (list.stream().noneMatch(i -> i.getId().equals(cartItem.getId()))) {
